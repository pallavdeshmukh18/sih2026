...
export default function ClinicalSummaryCard({
  ...
  return (
    ...
    >
      ...
      {/* Extract Executive Summary from markdown if present */}
      {(() => {
        let extractedExecSummary = null;
        let cleanedSummaryText = summary;

        if (typeof summary === "string" && summary.includes("## Executive Summary")) {
          const parts = summary.split("## Executive Summary");
          if (parts.length > 1) {
            const splitContent = parts[1].split(/## [A-Z]/);
            const afterHeader = splitContent[0];
            extractedExecSummary = afterHeader.replace(/^[\s\n]+|[\s\n]+$/g, "").trim();
            
            // Remove the Executive summary section from the rest of the text
            const firstPart = parts[0].trim();
            const restPart = summary.substring(summary.indexOf(afterHeader) + afterHeader.length).trim();
            cleanedSummaryText = firstPart + (firstPart && restPart ? "\n\n" : "") + restPart;
          }
        }

        return (
          <>
            {extractedExecSummary ? (
              <div style={{ background: "#f8fafc", border: "1px solid #cbd5e1", borderRadius: "12px", padding: "14px 16px", marginBottom: "16px", borderLeft: "4px solid #0f766e" }}>
                <strong style={{ fontSize: "13px", fontWeight: "700", color: "#0f172a", display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px" }}>
                  <Stethoscope size={15} /> Attending Physician Executive Summary
                </strong>

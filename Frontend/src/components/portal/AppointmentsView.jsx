...
export default function AppointmentsView({ doctor = false }) {
 ...
 return <div className={ui.page}>
  <header className={ui.header}><div><div className={ui.eyebrow}>{doctor?'CLINICAL SCHEDULE':'YOUR CARE PLAN'}</div><h1>{doctor?'Today’s schedule':'Appointments'}</h1><p>{doctor?'Manage today’s consultations and patient visits.':'Plan upcoming care and review past consultations.'}</p></div><button className={ui.primary}><Plus size={15}/>{doctor?'New appointment':'Book appointment'}</button></header>
